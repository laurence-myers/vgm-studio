...
//!
//! `dro-core` is integer-dominated and has no platform-specific paths, so byte
...
use dro_core::convert::dro_to_vgm;
use dro_core::io::{read_song, write_song};
use dro_core::vgm::io as vgm_io;
...
fn delete_and_undo_round_trip() {
    use dro_core::undo::{DeleteInstructions, UndoController};
