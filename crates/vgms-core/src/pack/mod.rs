...
//!
//! Two concerns split out into their own files: [`naming`] (what a pack's files
//! are called) and [`readiness`] (the checks a submission must pass). Both are
//! `pub mod` with no re-export, so callers reach their items through
//! `pack::naming::` and `pack::readiness::`.
...
use self::naming::title_from_filename;
