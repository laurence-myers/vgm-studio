...
use dro_core::{Song, slide_index_past_deletion};
...
mod tests {
    ...
    fn from_song_adopts_a_vgms_loop_region() {
        ...
        let mut song = dro_core::convert::dro_to_vgm(&tone_song()).unwrap();
        let len = song.len();
        {
    ...
    fn from_song_falls_back_to_the_whole_song() {
        ...
        let vgm = dro_core::convert::dro_to_vgm(&tone_song()).unwrap();
