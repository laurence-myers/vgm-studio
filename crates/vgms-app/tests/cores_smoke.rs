...
fn vgm_file(chips: &[(vgms_core::ChipKind, u32)], stream: &[u8]) -> Arc<vgms_core::vgm::VgmFile> {
...
fn the_default_sn76489_is_libvgms_and_makes_sound() {
    ...
    let mut engine = vgms_synth::VgmEngine::new(file, 44_100);
...
fn libvgm_leads_every_chip_it_serves_and_opl_is_untouched() {
    ...
    for chip in vgms_core::ChipKind::all() {
        ...
        assert!(
            default.build().is_some() || !matches!(default.make, vgms_synth::CoreMaker::Generic(_)),
