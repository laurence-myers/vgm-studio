//! An ad-hoc harness: renders one file through the generic engine and reports
//! its level. Point `SCRATCH_FILE` at a `.vgm`/`.vgz` and run with `--nocapture`;
//! without the variable the test is a no-op, so the suite stays green everywhere.
//!
//! Lives in `vgms-app` rather than `vgms-synth` because it needs the real chip
//! cores installed. `vgms-synth`'s own test binary registers none, so there the
//! render is always silence and the `peak > 500` assertion could never hold --
//! the test could only ever pass by being skipped.
use std::sync::Arc;
