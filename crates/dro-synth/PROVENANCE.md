...
| `cores/k051649.rs` | K051649 (SCC), K052539 (SCC+) | clean-room | The documented register windows: five 32-byte signed wavetables, 12-bit periods, 4-bit volumes, the enable mask, and the SCC's channel-5-plays-table-4 economy | n/a — no code read | MIT OR Apache-2.0 | n/a. The variant bit selects the SCC+, whose channel 5 owns its table and answers the port-4 window; both directions of the sharing are pinned by tests. Not modelled: the deformation/test register, the memory-mapper half (silent). |
| `cores/rf5c68.rs` | RF5C68, RF5C164 | clean-room | The documented register map (as MAME's `rf5c68.cpp`, BSD-3-Clause, documents it): eight channels from 64 KiB RAM through a 4 KiB banked window, 16.11 address steps, sign-magnitude samples, the `0xFF` loop marker | n/a — no code read | MIT OR Apache-2.0 | n/a. One core, both kinds: the parts answer the same registers. RAM uploads arrive as `0xC0` blocks and as `0xC1`/`0xC2` pokes, which the stream decoder hands over on port 1 so they cannot collide with the register file. Sign-magnitude and the marker semantics are each pinned by a test. |
| `cores/k054539.rs` | K054539 | clean-room | The documented register map (as MAME's `k054539.cpp`, BSD-3-Clause, documents it), scoped by a register histogram over the corpus rips | n/a — no code read | MIT OR Apache-2.0 | n/a. Three stated approximations: loops jump on the end marker unconditionally (the corpus keys everything off explicitly), reverb is ignored, pan is linear. The volume curve (-36 dB per 0x40) is compounded at const time and regenerated in a test. |
| `cores/c140.rs` | C140, C219 (approximate) | clean-room | The documented register map and mu-law segment table (as MAME's `c140.cpp`, BSD-3-Clause, documents them); the per-board address folds the VGM header's `c140_type` selects | n/a — no code read | MIT OR Apache-2.0 | n/a. System 2 and System 21 address folds implemented and pinned by test; the C219's own group-bank scheme is approximated as linear addressing, a stated gap. The mu-law table is regenerated from its doubling construction in a test. |
| `cores/c352.rs` | C352 | clean-room | The documented register and flag layout (as MAME's `c352.cpp`, BSD-3-Clause, documents them); the rate from the header's own divider field | n/a — no code read | MIT OR Apache-2.0 | n/a. Loops, reverse playback, phase inversion and the shared noise generator are modelled; the mu-law curve is a piecewise-linear reconstruction regenerated in a test; rear outputs mix into the front pair at half weight; FM/link/filter flags unmodelled. The `0xE1` command decode was corrected to big-endian operands — the corpus's own streams arbitrated (voice-times-eight register grid). |
| `cores/qsound.rs` | QSound | clean-room | The documented register interface the DSP program serves (as community documentation and MAME's `qsound.cpp`, BSD-3-Clause, describe it) | n/a — no code read | MIT OR Apache-2.0 | n/a. The sixteen PCM voices with pitch/loop/pan; the `0xC4` command normalised in the stream decoder (register and 16-bit value trade places relative to the rest of its range). Stated gaps: the echo and filter stages (the "Q"), and the three ADPCM effect voices. |
| `cores/y8950.rs` | Y8950 (MSX-AUDIO) — **written, not yet registered** | assembled | The Nuked-OPL3 port that serves the OPL path (LGPL, behind the `nuked-opl` feature) + this project's `adpcm::DeltaT` | as the `nuked_opl3` crate | LGPL-2.1-or-later (via the feature) | The Y8950 is one of the OPL chips, and the registry invariant — OPL is listed but never buildable for `VgmEngine` — is load-bearing for every `playability` caller. Registering this core without auditing that routing would send Y8950 files down two paths at once; the audit is its own step. Tests keep the core honest meanwhile. |
| `cores/segapcm.rs` | Sega PCM (315-5218) | clean-room | The documented register-RAM layout (as MAME's `segapcm.cpp`, BSD-3-Clause, documents it); the bank fold from the header's interface word | n/a — no code read | MIT OR Apache-2.0 | n/a. The register file is kept as the raw 256-byte RAM the driver writes; channels are read out at render time, and the chip sets its own stop bits, which the tests observe. |
| `cores/ga20.rs` | Irem GA20 | clean-room | The documented register map (as MAME's `iremga20.cpp`, BSD-3-Clause, documents it) | n/a — no code read | MIT OR Apache-2.0 | n/a. Unsigned samples with the `0x00` end marker (no loops — the driver replays), the rate register a divider; both pinned by tests. |
| `cores/ymz280b.rs` | YMZ280B | clean-room | The documented banked register file (as MAME's `ymz280b.cpp`, BSD-3-Clause, documents it) | n/a — no code read | MIT OR Apache-2.0 | n/a. Its ADPCM is the OKI ladder into a **16-bit clamping** accumulator — distinct from both the OKIM chips' 12-bit clamp and the OPN family's 12-bit wrap, pinned by test. DSP/IRQ block unmodelled. |
| `cores/k053260.rs` | K053260 | clean-room | The documented register map (as MAME's `k053260.cpp`, BSD-3-Clause, documents it) | n/a — no code read | MIT OR Apache-2.0 | n/a. The first eight addresses are CPU mailboxes, pinned as silent by a test; pitch is a divider; the DPCM shares the K054539's delta shape; the control register is deliberately unread. |
| `cores/wonderswan.rs` | WonderSwan | clean-room | The community hardware documentation (WSMan/Sacred Tech), as MAME's `wswan.cpp` also records it | n/a — no code read | MIT OR Apache-2.0 | n/a. Four packed-nibble wavetables in banked RAM, the channel-2 voice DAC, channel-4 noise and the channel-3 sweep; HyperVoice and output mixing unmodelled. |
| `cores/vsu.rs` | VSU (Virtual Boy) | clean-room | Nintendo's developer documentation as the community preserves it (ares and MAME record the same layout) | n/a — no code read | MIT OR Apache-2.0 | n/a. Five 6-bit wavetables, envelopes with repeat, the noise channel; channel 5's sweep/modulation unit is a stated gap. |
| `cores/saa1099.rs` | SAA1099 | clean-room | The Philips datasheet | n/a — no code read | MIT OR Apache-2.0 | n/a. Six squares (octave doubling pinned by test), two noise generators, the two envelope units on channels 2/5 simplified to the shape families the corpus uses; the master enable honoured because the datasheet defines the chip as silent without it. |
| `cores/pwm.rs` | 32X PWM | clean-room | Sega's 32X hardware manual as the community preserves it | n/a — no code read | MIT OR Apache-2.0 | n/a. A pulse-width DAC pair, not a synthesizer: duty against cycle, centred. Stated approximation: the rate is fixed at the standard clock/1042, so an unusual programmed cycle shifts pitch rather than rate. |
| `cores/es5503.rs` | ES5503 (Apple IIgs DOC) | clean-room | The IIgs hardware reference and Ensoniq's datasheet, as MAME's `es5503.cpp` also records them | n/a — no code read | MIT OR Apache-2.0 | n/a. Unsigned samples with the zero-byte stop code (pinned by test); the enable count divides the scan rate (pinned); swap mode plays as loop and output is mono — both stated. |
| `cores/ymf278b.rs` | YMF278B (OPL4), wave side | clean-room | Yamaha's OPL4 application manual as the community preserves it; MAME's `ymf278b.cpp` records the same layout | n/a — no code read | MIT OR Apache-2.0 | n/a. Wave headers read from the ROM itself (format/start/loop/negated-end, pinned by test); 8/12/16-bit formats including the packed-12 trio. Stated: the FM half (an OPL3 behind ports 0-1) is unmodelled, the envelope is simplified to instant attack + fade-out release, pan is linear. |
| `cores/x1010.rs` | X1-010 (Seta) | clean-room | The register layout as MAME's `x1_010.cpp` documents it, scoped by a register histogram over the corpus (both modes heavily used) | n/a — no code read | MIT OR Apache-2.0 | n/a. The address space *is* the sound RAM: channel files, envelope tables and waveforms all through one command (arbitrating this chip's writes is also what exposed the `0xC5`-`0xC8` big-endian decode fix). PCM self-stops at its end page and clears its own key bit; the wavetable mode reads RAM waves through RAM envelopes. Stated: the envelope walk rate is a linear approximation. |
| `cores/multipcm.rs` | MultiPCM (YMW258-F / 315-5560) | clean-room | The register interface as MAME's `multipcm.cpp` documents it; the write-port order and the Sega banking calibrated against the corpus's own streams | n/a — no code read | MIT OR Apache-2.0 | n/a. The OPL4-style ROM sample headers; the 28-of-32 slot encoding; bit-20 addresses pass through the bank register (power-on default 0x180000, arbitrated by OutRunners' bankless stream; Daytona's explicit 0xC3 confirms the rule). Envelope simplified as the OPL4 core's; LFO unmodelled. |
| `cores/upd7759.rs` | uPD7759 | clean-room + documented tables | The block format, step matrix and state walk as MAME's `upd7759.cpp` (BSD-3-Clause, Buchmueller/Balfour/Cohen/Galibert/Giles) documents them; tables transcribed with the notice kept | MAME master `upd7759.cpp` (fetched 2026-07-28) | MIT OR Apache-2.0 (tables BSD-3 attributed) | Both drive modes: the slave FIFO stream that dominates the corpus, and master mode reading the ROM through its phrase table — whose pointers start at offset 5, after the `5A A5 69 55` signature the corpus ROMs carry in plain sight. DRQ pacing unmodelled (stated). |
