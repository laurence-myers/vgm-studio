//! DRO capture: re-record a (muted) playthrough as a new DRO v2 file (Python
//! `dro_capture.py`).
//!
//! This is what `dro_split --dro` uses to split a song into one DRO per channel.
//! It renders no audio: it walks the instruction stream, applies the same
//! [`Muting`] gate the player does, and re-emits the surviving register writes and
//! delays as a DRO v2 stream. Delays are re-encoded into v2 short/long opcodes;
//! the total timing is preserved.

use std::collections::HashMap;

use dro_core::{Bank, DroDataV2, DroInstruction, Error, OplType, Result, Song, SongData};
...
/// Re-records `song` through `muting` as a new DRO v2 song named `name`.
///
...
///
/// # Errors
/// If `song` is a VGM (its sample delays have no DRO encoding), or the capture
/// exceeds the 128-code codemap limit.
...
mod tests {
    ...
    fn the_capture_round_trips_through_the_dro_writer() {
        let song = read_song("lsl3_score_up_dro2.dro", DRO_FIXTURE).unwrap();
        let captured = capture(&song, Muting::all(), "out.dro".to_owned()).unwrap();
        let bytes = dro_core::io::write_song(&captured).unwrap();
