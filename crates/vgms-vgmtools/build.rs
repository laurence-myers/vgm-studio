...
fn main() {
    ...
    // On wasm32 there is nothing to build: the wasm dependents (vgms-web) use
    // only the target-independent pipeline. The tools themselves become
    // wasm32-wasip1 command modules built by `tools/build-wasi-tools.ps1`,
    // outside cargo entirely -- unmodified POSIX C over the real wasi-libc.
    // See `docs/vgm-multichip-2026-07/OPTIMIZER-WASM-PLAN.md`.
