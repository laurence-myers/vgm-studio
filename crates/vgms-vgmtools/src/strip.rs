...
//! a question about the command stream that `vgms_core` already models, while
...
const STRIP_NAMES: &[(ChipKind, &str)] = &[
    ...
    // chips `vgms_core::ChipKind` does not model, so nothing here can name them;
...
pub fn unused_chips(vgm: &[u8]) -> Vec<ChipKind> {
    let Ok(file) = vgms_core::vgm::file::read("checking.vgm", vgm) else {
        return Vec::new();
    };
