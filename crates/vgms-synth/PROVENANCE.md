...
| `vgms-cores-gpl` → `lle_opm.rs` | YM2151, YM2164 -- **the LLE tier's first core** | **submodule + `cc`** | `vendor/upstream/ym2151-lle` (`nukeykt/YM2151-LLE`), `fmopm.c` + `fmopm.h` | `efa722f342119b69457e0e02a007449c5baac698` | **GPL-2.0-or-later** | **Compiled unmodified.** A die simulation from the decap, not a behavioural model: the API is the package's pins, so the wrapper drives the bus electrically (CS/WR asserted across master clocks, IC held low to reset) and decodes the YM3012 serial DAC stream itself -- 13 bits at half the master clock, offset-binary mantissa then exponent, framed backwards from each S/H falling edge. The framing was pinned with a pin probe (the idle word decodes to exactly zero) and is regression-tested. `realtime: false`: below realtime on today's CPUs, never the default -- but a user's explicit pick is honoured for playback too. Pin access happens in `shim/lle_opm.c` against the upstream's own header, keeping the no-struct-mirroring rule. |
| `vgms-cores-gpl` → `lle_opl2.rs` | YM3812 (die); YM3526 and Y8950 rendered the same -- stated approximations (no waveform-select gate difference asserted; **no ADPCM**, so a Y8950 rip loses its sample channel) | **submodule + `cc`** | `vendor/upstream/ym3812-lle` (`nukeykt/YM3812-LLE`), `fmopl2.c` + `fmopl2.h` | `a61c464b751c5216bf16448c272d4cb8ac9e0299` | **GPL-2.0-or-later** | **Compiled unmodified.** The OPL2 die from Travis Goodspeed's decap, in the shared `opl3` picker slot beside Nuked-OPL3/CQM (registered for the OPL2-generation chips only -- an OPL3 song needs the second register bank the die lacks). This upstream reads the master-clock pin from its input struct rather than a parameter, so the shim presents the level and clocks in one call. Mono serial DAC in the YM3012-family float format, one bit per `SY` bit clock (clock/4), word ending at the `SH` strobe's fall -- pinned by the idle-decodes-to-zero probe. Bus pacing: address bytes wait 32 master clocks (wider than the datasheet's 12: `data_latch` is shared between address and value bytes, and the die's commit chain takes ~16 clocks -- racing it decodes the value byte as the register select), value bytes the datasheet's 84. `realtime: false`, as every die. |
| `vgms-cores-gpl` → `opl2_lite.rs` | YM3812 (behavioural), YM3526 and Y8950 rendered the same -- the OPL2 generation's **realtime** authenticity option, between Nuked-OPL3's compat mode and the YM3812 die sim | **submodule + `cc`** | `vendor/upstream/nuked-opl2-lite` (`nukeykt/Nuked-OPL2-Lite`), `opl2.c` + `opl2.h` | `61f0c5657fdadf2c0614abe7a0ddff3417dc4476` | **GPL-2.0-or-later** -- the repo's LICENSE file says LGPL-2.1 but every source header says GPL-2.0-or-later; this project honours the stricter claim, hence this crate | **Compiled unmodified.** Same author, same API shape as Nuked-OPL3 (reset at an output rate, immediate + buffered writes with the two-sample `writebuf` spacing, internal resampler), hosted as an `OplChip` through the `OplCoreAdapter`. Mono, mirrored to both sides. One register bank: bank-1 writes are dropped, not aliased -- a real OPL2 has no A1 pin. The upstream struct carries self-pointers planted by `OPL2_Reset`; safe because the opaque allocation never moves. |
| `vgms-cores-gpl` → `lle_opn.rs` | YM2203 (die) -- **the chip's first alternative core here** (libvgm alone served it) | **submodule + `cc`** | `vendor/upstream/ym2203-lle` (`nukeykt/YM2203-LLE`), `fmopn.c` + `fmopn.h` | `817387fd89affd0db5331034d6ae04d919e25a5e` | **GPL-2.0-or-later** | **Compiled unmodified.** The YM2203C from John McMaster's decap: FM leaves as the YM3014 float stream on `OPO` (`SY` bit clock, `SH` framing, zero trail -- the OPL2 die's convention), the licensed SSG's three channels leave on analog pins the shim sums at the OPNA wrapper's 8000 scale, and the GPIO ports are tied low. One FM sample per 72 master clocks at the default /6 prescale; a song that reprograms the prescaler shifts the die's cadence under a fixed output rate, a stated approximation. Bus pacing 48/96 master clocks (the shared-data-latch commit margin, /6-prescale sized). |
| `vgms-cores-gpl` → `lle_opn2l.rs` | YMF276 (die) -- the OPN2's serial-DAC sibling, the *clean* witness beside the 2612 die's ladder distortion; a YM3438-variant song runs the same configuration (the upstream's YM3438 build drives no output pins), a stated approximation | **submodule + `cc`** | `vendor/upstream/ymf276-lle` (`nukeykt/YMF276-LLE`), `fmopn2.c` + `fmopn2.h` | `485fb53c6de7de299a73b97ef29a480a2daeb5e1` | **GPL-2.0-or-later** | **Compiled unmodified.** Two traps pinned: (1) unlike every sibling die this upstream models its pins *logically* (`ic` true = reset asserted, `cs && wr` = write on), so the shim inverts once and the Rust driver keeps the electrical convention; (2) the die enforces the OPN2's real BUSY window -- writes are dropped for ~32 internal cycles (192 master clocks) after every data write -- so value bytes wait two whole samples where the 2612 die's wrapper waits one. Serial DAC: 16-bit two's-complement words MSB first on `SO`, one word starting flush with each `WCO` rise (`BCO` bit clock, `LRO` high at the rise = left, pinned by the pan test). |
| `vgms-cores-gpl` → `lle_opl3.rs` | YMF262 (die); the whole OPL family served, as Nuked-OPL3/CQM serve it -- an OPL2 song on OPL3 silicon is the SB16 experience (an OPL2-generation clock runs the die at four times itself, the hardware upgrade path's arithmetic; **no Y8950 ADPCM**, the same stated approximation every OPL3-family core makes) | **submodule + `cc`** | `vendor/upstream/ymf262-lle` (`nukeykt/YMF262-LLE`), `fmopl3.c` + `fmopl3.h` | `152ee10d095b2e22eba60433877af47c78744736` | **GPL-2.0-or-later** | **Compiled unmodified.** The OPL3 die from John McMaster's decap, in the shared `opl3` picker slot behind the modelled default. Same input-struct clock quirk as the OPL2 die (the shim presents the level). Four 16-bit linear words per sample in the YAC512's offset-binary format on two serial lines (`DOAB` multiplexing A/B, `DOCD` C/D), framed by `SMPAC`/`SMPBD`; the strobe's fall trails the word by one bit clock (the OPNA's convention, not the OPL2's zero-trail), pinned by the idle-decodes-to-zero probe. The wrapper mixes `left = A + C`, `right = B + D`. Bus pacing: 64 master clocks per byte, twice the datasheet's 32, clear of the die's clock/8 commit chain (the shared-data-latch race the OPL2 die demonstrated). No stereo-ext panpots: that is Nuked-OPL3's extension, and the die predates it, so `channel_pan: false` and muting rides the OPL write gate. `realtime: false`, as every die. |
