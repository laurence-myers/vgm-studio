...
use dro_core::{ChipKind, Song, VgmFile};
...
/// [`dro_synth::AudioSource`] splits them; this is that split made at the
...
impl LoadedSong {
    ...
    #[must_use]
    pub fn audio_source(self) -> dro_synth::AudioSource {
        match self {
            Self::Opl(song) => dro_synth::AudioSource::Opl(Arc::new(song)),
            Self::Vgm(file) => dro_synth::AudioSource::Vgm(Arc::new(*file)),
        }
    }
    ...
    /// [`dro_synth::playability`] wants to hear about.
    #[must_use]
...
pub fn warn_missing_cores(chips: &[ChipKind], all_silent: &str) -> Result<()> {
    match dro_synth::playability(chips) {
        dro_synth::Playability::None => anyhow::bail!(
...
pub fn read_any_song_from_path(path: &Path) -> Result<LoadedSong> {
    ...
    if lower.ends_with(".vgm") || lower.ends_with(".vgz") {
        let file = dro_core::vgm::file::read(name, &bytes)?;
