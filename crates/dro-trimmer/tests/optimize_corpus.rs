...
use dro_core::{Bank, DroInstruction, Song};
use dro_synth::{FrameClock, NATIVE_SAMPLE_RATE, NukedOpl3, OplChip, render_wav};
...
fn optimise_the_whole_corpus() {
    ...
    for path in &songs {
        ...
        let song = match dro_core::io::read_song(&name, &bytes) {
            Ok(song) => song,
            Err(_) => {
                unreadable += 1;
                continue;
