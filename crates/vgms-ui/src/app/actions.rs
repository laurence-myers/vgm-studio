...
impl VgmStudioApp {
    ...
    fn handle_pack_action(&mut self, action: PackAction) {
        match action {
            ...
            PackAction::OpenTrackOptimizeOptions(index) => self.open_track_optimize_options(index),
            PackAction::SetTrackOptimizeOptions { file_name, options } => {
                self.set_track_optimize_options(file_name, options);
            }
