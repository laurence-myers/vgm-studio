...
| Term | POS | Approved meaning | Source / notes |
|---|---|---|---|
| **engine** | n | The one pull-based playback state machine: **`VgmEngine`** walks any VGM through `ChipCore` voices and mixes them. Every document plays through it — a DRO is projected to its VGM first (`opl_song_to_vgm_file`), so live playback, WAV render, peak scan, waveform and CLI render all share this path. `engine.rs` is now just the clock, loop and mix-vocabulary types it uses. | [vgm_engine.rs:157](crates/vgms-synth/src/vgm_engine.rs), [engine.rs:1](crates/vgms-synth/src/engine.rs), [vgms-audio-native/lib.rs:203](crates/vgms-audio-native/src/lib.rs) |
