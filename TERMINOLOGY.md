...
| **shadow** | v/n | Keep a private copy of register state so it can be resynced or restored (the engine's `c0_shadow`, the RetroWave chip's board shadow). | [engine.rs:402](crates/vgms-synth/src/engine.rs) |
...
| **wrap** ⚠ | v | One jump back from loop end to loop start during playback (`wraps_remaining`, `owes_a_wrap`). Do not use for core decorators (`Leveled::wrap`) in prose. | [engine.rs:572](crates/vgms-synth/src/engine.rs) |
...
| **`Muting`** ⚠ | n (type) | The OPL *document* vocabulary: 18-bit channel mask + per-bank 0xBD masks. **Set bit = audible.** | [engine.rs:80](crates/vgms-synth/src/engine.rs) |
...
| **`Panning`** / **`ChipPanning`** | n (types) | Same pairing for stereo: OPL panpot-byte image (Original/Custom) vs per-chip position arrays (−0x100..0x100). `opl_chip_panning`/`pan_of` convert. | [engine.rs:232](crates/vgms-synth/src/engine.rs), [chip_mix.rs:96](crates/vgms-synth/src/chip_mix.rs) |
...
| **gate** | v | Filter register writes so a muted channel never sounds (drop key-ons, clear key bits, force volumes) instead of masking rendered output. Hosts: `ChannelGate`, `GatedCore`, song gate. Opposite: **native mute** (the core masks its own output). | [engine.rs:167](crates/vgms-synth/src/engine.rs), [channel_gate.rs:72](crates/vgms-synth/src/channel_gate.rs) |
...
| Term | POS | Approved meaning | Source / notes |
|---|---|---|---|
| **engine** | n | A pull-based playback state machine. Two exist: **`PlayerEngine`** (plays a DRO `Song`; today *offline-only* — WAV render, peak scan, waveform, CLI render) and **`VgmEngine`** (walks any VGM through `ChipCore` voices; *all live playback*, DROs included, via projection). "The engine" unqualified is ambiguous — name it. | [engine.rs:389](crates/vgms-synth/src/engine.rs), [vgm_engine.rs:159](crates/vgms-synth/src/vgm_engine.rs), [vgms-audio-native/lib.rs:203](crates/vgms-audio-native/src/lib.rs) |
...
| **render** ⚠ | v | Generate audio frames. Three senses (live pull, a core's native-rate frames, offline WAV/waveform export) plus egui painting; qualify when crossing layers. | [engine.rs:627](crates/vgms-synth/src/engine.rs) |
| **transport** | n | The realtime playback path; builds only cores that keep up (`core_for_realtime`), falling past the offline-only LLE tier. | [chip.rs:180](crates/vgms-synth/src/chip.rs) |
| **seek replay** | n | Rebuilding chip state after a seek by rewriting every prior write with no samples rendered (`PlayerEngine`); `VgmEngine` instead folds state and executes the restore set. | [engine.rs:668](crates/vgms-synth/src/engine.rs) |
...
| **`RecordingChip`** (type) | two distinct test doubles (ChipCore in chip.rs; OplChip privately in engine.rs) | Known duplicate name; rename candidate when touched. |
