// nuked-opl3
// A Rust implementation of the Nuked-OPL3 chip core with a small device wrapper.
//
...
#[cfg(all(test, feature = "c-reference-tests"))]
#[allow(dead_code, non_snake_case)]
mod bindings;
mod core;

/// The upstream [Nuked-OPL3](https://github.com/nukeykt/Nuked-OPL3) version
/// this port is derived from.
pub const UPSTREAM_VERSION: &str = "1.8";
...
#[derive(Debug)]
/// The `OplError` enum represents errors that can occur when using the `nuked-opl3` library.
pub enum OplError {
...
impl std::fmt::Display for OplError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            OplError::RegisterOutOfRange => write!(f, "Register number out of range"),
            OplError::MutexLockFailed => write!(f, "Failed to lock mutex"),
        }
    }
}
...
/// The `Opl3Chip` struct provides a safe interface for interacting with the Nuked-OPL3-compatible core.
pub struct Opl3Chip {
    chip: core::Chip,
}
...
impl Opl3Chip {
    ...
    pub fn new(sample_rate: u32) -> Self {
        Opl3Chip {
            chip: core::Chip::new(sample_rate),
        }
    }
    ...
    ///
    /// Internally, this calls the 4-channel generator and returns samples for the first 2 channels.
    ///
    /// # Arguments
    ///
    ...
    pub fn generate_resampled(&mut self, sample: &mut [i16]) -> Result<(), OplError> {
        ...
        self.chip.generate_resampled(sample);
        Ok(())
    }
    ...
    pub fn generate_stream(&mut self, buffer: &mut [i16]) -> Result<(), OplError> {
        ...
        self.chip.generate_stream(buffer, buffer.len() / 2);
        Ok(())
    }

    /// Return the number of slots that are not fully released and silent.
    ///
    /// This is intended for lightweight playback diagnostics and status UIs.
    /// It does not affect synthesis state.
    pub fn active_voice_count(&self) -> usize {
        self.chip.active_voice_count()
    }
    ...
    pub fn generate_4ch_resampled(&mut self, sample: &mut [i16]) -> Result<(), OplError> {
        ...
        self.chip.generate_4ch_resampled(sample);
        Ok(())
    }
    ...
    pub fn generate_4ch_stream(
        ...
        self.chip
            .generate_4ch_stream(buffer1, buffer2, buffer1.len() / 2);
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn stats_track_generated_samples() {
        let mut device = Opl3Device::new(44_100);

        assert_eq!(device.stats().samples_generated, 0);

        let mut sample = [0i16; 2];
        device.generate(&mut sample).unwrap();
        assert_eq!(device.stats().samples_generated, 1);

        let mut buffer = [0i16; 8];
        device.generate_samples(&mut buffer).unwrap();
        assert_eq!(device.stats().samples_generated, 5);

        let mut undersized = [0i16; 1];
        assert!(matches!(
            device.generate(&mut undersized),
            Err(OplError::BufferUndersized)
        ));
        assert_eq!(device.stats().samples_generated, 5);

        device.reset(None).unwrap();
        assert_eq!(device.stats().samples_generated, 0);
    }

    #[cfg(feature = "c-reference-tests")]
    #[path = "c_reference_tests.rs"]
    mod c_reference;
}
