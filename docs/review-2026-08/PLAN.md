...
| **mg-2** | **PARTLY SUBSUMED + PARTLY STAGE-K-GATED.** The dead old-path helpers (`read_uncompressed`, `resolve_loop_point`/`resolve_loop_end`, private `opl_type_of`, `VgmData::read_from_stream`) were already deleted by **mg-1**, which is where they became dead. **KEEP `io::write`** (D20 keeps `SongData::Vgm` a writable carrier — the "delete `io::write`" wording was an error). Remaining, both optional-for-Stage-I: **(a)** remove the *public* `vgm::io::read` wrapper — now a thin `file::read + to_song` projector — by folding it into `read_song`'s VGM branch and retargeting its ~15 (nearly all test) callers to `read_song`/`file::read`; behaviour-neutral API hygiene. **(b) `read_song` DRO-only is STAGE-K-GATED**, not doable in Stage I: its production callers (`read_song_from_path`, the worklet, the web codec) pass both DRO and VGM and need a `Song`; they cannot move to `VgmFile` until the projection is retired (Stage K / mg-5's `DocSource`). Forcing DRO-only now would duplicate the format dispatch at every caller. The D20 "one VGM reader" target is already met by mg-1 (`file::read` is the sole reader; `io::read` merely projects it). | contained |
