...
> native-only).
>
> **Progress 2026-07-31 (cont.): wt-9 DONE (investigation), verdict GO-deferred.**
> (a) `vgms-retrowave` now wasm-checks: `serialport`/`rtrb` moved under a
> `cfg(not(wasm32))` target table, and `device`/`player`/`test_tone` gated off
> wasm, leaving the pure `protocol`/`commands`/`chip` compiling for wasm32 (in the
> CI wasm-clean list; native still green, 53 tests). (b) A no-Rust hardware spike
> (`docs/web-target-2026-07/web-serial-spike/serial-hardware-spike.html`) replays
> a **provably-correct** bring-up + chord wire dump (generated by the real
> protocol via `cargo run -p vgms-retrowave --example dump_wire`) over Web Serial;
> hardware-gated (verify by listening). (c) A timing rehearsal (same dir,
> **measured in headless Chromium**): the naive one-wake-per-quantum loop the
> native pump uses *breaches* the 250 ms tolerance in a browser (mean lag 128 ms,
> 49 resets -- `setTimeout` clamps below ~1.3 ms), but a **batched render-to-
> deadline** pump on an 8 ms timer holds it (mean 0.66 ms, worst 7.7 ms, 0
> resets). **Verdict: GO, deferred.** Web Serial output is feasible -- the protocol
> is already wasm-clean and transport-agnostic -- but it needs a follow-up design
> for a worker-hosted *batched* pump; the open frictions are the `SerialIo: Send`
...
### wt-9 go/no-go (2026-07-31): **GO, deferred**

All three items done and committed (see `web-serial-spike/README.md` for the
detail and the runnable pages):

- **(a) crate hygiene — DONE.** `serialport`/`rtrb` moved under a
  `[target.'cfg(not(target_arch = "wasm32"))'.dependencies]` table;
  `device`/`player`/`test_tone` gated off wasm; the pure `protocol`/`commands`/
  `chip` compile for wasm32. `cargo check -p vgms-retrowave --target
  wasm32-unknown-unknown` passes (added to the CI wasm-clean list); native stays
  green (53 tests).
- **(b) hardware spike — DONE (hardware-gated).**
  `web-serial-spike/serial-hardware-spike.html` `requestPort`s (04D8:E966), opens
  the CDC port, and replays a bring-up + chord. The wire bytes are generated by
  the real protocol (`cargo run -p vgms-retrowave --example dump_wire`), not
  re-derived in JS, so they are provably correct. Verified by listening — awaits a
  session with the board attached.
- **(c) timing rehearsal — DONE, measured in headless Chromium** (4000 quanta):
  the native one-wake-per-quantum loop **breaches** the 250 ms tolerance in a
  browser (mean lag 128 ms, 49 deadline resets — `setTimeout` clamps below the
  ~1.3 ms quantum), but a **batched render-to-deadline** pump on an 8 ms timer
  **holds** it (mean 0.66 ms, p99 1.28 ms, worst 7.7 ms, 0 resets).

**Verdict: GO, deferred.** Web Serial output is feasible — the protocol layer is
already wasm-clean and transport-agnostic (`SerialIo`), and a batched
worker-hosted pump tracks real time comfortably. It is **not built in this
programme**; a follow-up design doc should cover the frictions the crate map and
the rehearsal surfaced:

1. `SerialIo: Send` fights a JS-handle transport (a wasm impl is `!Send`) — the
   seam is the JS boundary, not the trait.
2. `Device::initialise` → `reset_chip` sleeps 200 ms synchronously; a web
   bring-up must make the settle async.
3. The pump must **batch** (render every due quantum per coarse wake), not port
   the per-quantum loop — the rehearsal is the evidence.
4. Background-tab timer throttling bunches writes (bounded by the deadline reset,
   but playback should pause/warn when the tab is hidden).
5. WritableStream backpressure is unmeasured without hardware.
